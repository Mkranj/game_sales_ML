...
Those with non-alphanumeric symbols
```{r}
games$unusual_title_symbol <- games$Name %>% mark_unusual_titles()
```

```{r}
unusual_titles <- group_by(games, unusual_title_symbol) %>%
  summarise(avg = mean(Global_Sales), 
            std = sd(Global_Sales),
            no = n())
...
unusual_titles 
```
This does appear to lower sales.
...
predictors <- c("Year_release_n", "Genre_F", "Rating_F", "dev_avg_income",  "subtitle_count",
                console_colnames, "Consoles_no", "unusual_title_symbol")
criterium <- "Global_Sales"

predictors_to_center <- c("Year_release_n", "dev_avg_income")
...
model_recipe <- recipe(model_formula, data = tt_training) |> 
  ...
  step_center(predictors_to_center, starts_with(c("Consoles_no", "subtitle_count"))) |> 
  step_scale(predictors_to_center, starts_with(c("Consoles_no", "subtitle_count")))
