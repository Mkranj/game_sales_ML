...
original$subtitled_name <- subtitled
original$subtitle_colon_count <- colon_count
```

```{r}
by_subtitle <- group_by(original, subtitled_name) %>%
  summarise(avg = mean(NA_Sales), 
            std = sd(NA_Sales),
            no = n())

no_subtitles <- group_by(original, subtitle_colon_count) %>%
  summarise(avg = mean(NA_Sales), 
            std = sd(NA_Sales),
            no = n())
